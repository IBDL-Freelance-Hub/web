...
export function RegistrationModal() {
  ...
  return (
    <div className="ov animate-in fade-in fixed inset-0 z-[1000] flex min-h-screen items-center justify-center overflow-y-auto bg-[#0a0a18]/65 p-4 backdrop-blur-[9px] duration-200 sm:p-8">
      <div className="sheet sheet--reg animate-in fade-in zoom-in-95 relative my-auto w-full max-w-[840px] overflow-hidden rounded-[24px] bg-white shadow-[0_28px_70px_rgba(20,20,40,0.16)] duration-300 sm:rounded-[34px]">
        {/* Close Button (.sheet__x) */}
        <button
          onClick={closeRegistration}
          aria-label="Close registration modal"
          className="sheet__x absolute end-4 top-4 z-20 grid h-10 w-10 cursor-pointer place-items-center rounded-full bg-white/90 text-[#16162c] shadow-sm transition-all hover:rotate-90"
        >
          <X className="h-5 w-5" />
        </button>

        {/* Modal Header (.rm__head) */}
        <div className="rm__head relative overflow-hidden bg-[#1d1d39] p-6 text-start sm:p-10">
        ...
        {typeof step === "number" && step <= 3 && (
          <div className="rm__prog flex items-center justify-between border-b border-[#e2e2ec] bg-[#f6f6fa] px-6 py-4 sm:justify-start sm:gap-8 sm:px-12">
            {/* Step 1 */}
            <div className="rm__pstep flex items-center gap-2.5">
              <div
                className={cn(
                  "rm__pnum grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold transition-all",
                  step === 1
              ...
              <span
                className={cn(
                  "rm__plab text-xs font-bold transition-all sm:text-sm",
