...
export function Navbar() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Brand Logo Image — refined compact size per user feedback */}
        <Link
          href="/"
        ...
        {/* Desktop Links with Active Rounded Pill Background */}
        <nav className="hidden items-center gap-1.5 text-xs sm:text-sm lg:flex">
          {navLinks.map((link) => {
        ...
        {/* Right Controls matching screenshot styling */}
        <div className="hidden items-center gap-3 lg:flex">
          ...
          {/* Join the Hub CTA with glowing red shadow */}
          <button
            type="button"
        ...
        <button
          ...
          className="rounded-lg p-2 text-slate-300 hover:text-white focus:outline-none lg:hidden"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="animate-in slide-in-from-top-2 space-y-4 border-b border-white/10 bg-[#141226] px-4 pt-2 pb-6 text-start lg:hidden">
          ...
          <div className="flex flex-col gap-3 border-t border-white/10 pt-4">
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <Link
                href="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="w-1/2 rounded-full border border-slate-700/80 bg-slate-900/60 px-4 py-2 text-center text-xs font-semibold text-white hover:bg-slate-800"
              >
                {locale === "ar" ? "تسجيل الدخول" : "Sign in"}
              </Link>
              <button
                ...
                className="bg-brand-secondary w-1/2 rounded-full px-4 py-2 text-xs font-bold text-white shadow-[0_0_15px_rgba(225,17,25,0.6)] hover:bg-red-600"
              >
                {locale === "ar" ? "انضم إلى المنصة" : "Join the Hub"}
              </button>
            </div>
