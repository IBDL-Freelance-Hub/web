...
export interface NextStepsHeaderProps {
  isInView?: boolean;
}

export function NextStepsHeader({ isInView = true }: NextStepsHeaderProps) {
  const { locale } = useLocale();
  const isArabic = locale === "ar";

  return (
    <div className="head max-w-[760px] text-start">
      {/* Eyebrow */}
      <div
        className={cn(
      ...
      >
        {isArabic
          ? "من التسجيل وحتى أول تجربة لك داخل المنصة."
          : "From registration to your first Hub experience."}
      </h2>

      {/* Lead */}
      <p
        className={cn(
          "lead mb-16 text-base leading-relaxed text-[#5f6368] transition-all delay-200 duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] motion-reduce:transform-none motion-reduce:opacity-100 motion-reduce:filter-none sm:text-[17px]",
          isInView
            ? "translate-y-0 opacity-100 filter-none"
            : "-translate-y-6 opacity-0 blur-[5px]"
        )}
      >
        {isArabic
          ? "رحلة بسيطة وموجهة — تربطك بأدوات التنمية والتعليم المتخصصة والموارد والفرص منذ لحظة انضمامك."
          : "A simple, guided journey — connecting you with specialized L&D tools, resources, and opportunities from the moment you join the Hub."}
      </p>
    </div>
  );
}
